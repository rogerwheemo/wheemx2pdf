...
def render_step_files_to_image(output_path: str, width: int = 800, height: int = 600,
    ...
    if not STEP_FILES_DIR.exists():
        print(f"[3D Model] ERROR: step_files folder not found at {STEP_FILES_DIR}")
        return False
    
    # Get unique STEP files (avoid duplicates)
    step_files_set = set()
    for pattern in ["*.step", "*.STEP", "*.stp", "*.STP"]:
        for f in STEP_FILES_DIR.glob(pattern):
...
def generate_pdf(values: dict, output_path: str, motor_name: str = "",
    ...
    elec_data = [
        ...
        ["Phase Resistance", f"{values['phase_resistance_ohm']*1000:.3f}", "mOhm"],
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        input_file = input("Enter the path to the JSON simulation output file: ").strip()
    ...
    output_filename = input_path.stem + f"_datasheet_{op_suffix}.pdf"
